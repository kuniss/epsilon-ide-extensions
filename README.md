...
If configured, the front implemented here is able to subsequently run the "real" generator to either generate in the final compiler in [D](https://dlang.org) or to report errors and warnings from the generator run.
...
* Gitpod support via the VS Code extensions available from [Eclipse Open VSX Registry](https://open-vsx.org/?search=grammarcraft&category=&sortBy=relevance&sortOrder=desc)
* Eclipse plugin (update site: http://epsilon.grammarcraft.de/)
